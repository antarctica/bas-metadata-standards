...
# BAS Metadata standards alias (Staging)
#
# This resource implicitly depends on the 'aws_cloudfront_distribution.bas-metadata-standards-staging' resource
...
resource "aws_route53_record" "bas-metadata-standards-staging" {
  ...
  records = [
    "${aws_cloudfront_distribution.bas-metadata-standards-staging.domain_name}",
  ]
}
