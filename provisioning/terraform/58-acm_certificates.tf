...
# metadata-standards-testing.data.bas.ac.uk
#
...
resource "aws_acm_certificate" "bas-metadata-standards-staging" {
  provider = "aws.us-east-1"

  domain_name       = "${aws_s3_bucket.bas-metadata-standards-staging.bucket}"
...
# metadata-standards-testing.data.bas.ac.uk
#
# This resource implicitly depends on the 'aws_acm_certificate.bas-metadata-standards-staging' resource
...
resource "aws_route53_record" "bas-metadata-standards-staging-cert" {
  zone_id = "${data.terraform_remote_state.BAS-CORE-DOMAINS.DATA-BAS-AC-UK-ID}"

  name = "${aws_acm_certificate.bas-metadata-standards-staging.domain_validation_options.0.resource_record_name}"
  type = "${aws_acm_certificate.bas-metadata-standards-staging.domain_validation_options.0.resource_record_type}"
  ttl  = 60

  records = [
    "${aws_acm_certificate.bas-metadata-standards-staging.domain_validation_options.0.resource_record_value}",
  ]
}
...
#
# This resource implicitly depends on the 'aws_acm_certificate.bas-metadata-standards-staging' resource
# This resource implicitly depends on the 'aws_route53_record.bas-metadata-standards-staging-cert' resource
# This resource relies on the AWS Terraform provider ('us-east-1' alias) being previously configured
#
...
#
# Tags are not supported by this resource
resource "aws_acm_certificate_validation" "bas-metadata-standards-staging" {
  provider = "aws.us-east-1"

  certificate_arn         = "${aws_acm_certificate.bas-metadata-standards-staging.arn}"
  validation_record_fqdns = ["${aws_route53_record.bas-metadata-standards-staging-cert.fqdn}"]
}
